...
- **Selezione controlli**: 34 verifiche suddivise in 3 categorie, ognuna attivabile/disattivabile:
  - **Hardware**: CPU, RAM (con conteggio slot occupati/liberi), dischi, salute SMART (tipo SSD/HDD), GPU, scheda madre/BIOS, rete, batteria, temperature, USB, **tastiera e mouse** (input, con errori driver), **test tasti bloccati** (rileva un tasto fisicamente incastrato, es. il classico "Canc bloccato"), **monitor e audio** (output, con errori driver), **monitoraggio prestazioni** (campiona CPU/RAM/disco per ~8 secondi per scovare picchi intermittenti), **test velocità disco reale** (scrive/legge un file da 100 MB).
